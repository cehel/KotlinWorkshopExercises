package com.zuehlke.NewsAnalyzerKotlin.common
...
object TestNewsArticle {

    inline fun <reified T> Gson.fromJson(json: String) = this.fromJson<T>(json, object: TypeToken<T>() {}.type)

    fun newsArticle(): NewsArticle? {
        val json = newsArticleString()
        val gson = GsonBuilder().registerTypeAdapter(
                LocalDateTime::class.java, JsonDeserializer<LocalDateTime> {
            json, typeOfT, context ->
            LocalDateTime.parse(json.asString, DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss'Z'"))
        }
        ).create()
        // // setDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'").create()
        val articles = gson.fromJson<NewsArticle>(json)
        return articles
    }

    fun newsArticleString(): String {
        return "{\n" +
                "  \"status\": \"ok\",\n" +
                "  \"totalResults\": 10,\n" +
                "  \"articles\": [\n" +
